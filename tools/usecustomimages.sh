...
if [ -n "${EMBER_BUILD_URL}" ]; then
	EMBER_IMAGE_URL=`curl -k ${EMBER_BUILD_URL} | egrep -o "http://.*tar.gz"`
fi
...
if [ -n "${EMBER_OPERATOR_BUILD_URL}" ]; then
	EMBER_OPERATOR_IMAGE_URL=`curl -k ${EMBER_OPERATOR_BUILD_URL} | egrep -o "http://.*tar.gz"`
fi
